...
its Rust and eBPF builds; `linux.bzl` consumes the same selected rustc and
matching sources for Rust-for-Linux without renaming or replacing Aya's
toolchain. Keeping this as a separate root module isolates Aya's nightly from
the stable minimum-version coverage in `e2e/`.
