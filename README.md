...
```starlark
# MODULE.bazel
bazel_dep(name = "rules_rs", version = "0.0.99")
...
The maintained kernels require at least Rust 1.78.0. A consumer may instead
register a newer stable toolchain or a pinned nightly such as
`version = "nightly/2026-06-24"`. rules_rs supplies rustc and its matching
sources as one toolchain. During actions, `linux.bzl` probes the selected rustc
release and embedded LLVM version, writes those exact values into the resolved
kernel config, and applies source-derived version predicates. There is no
separate checked-in Rust source archive or analysis-time compiler guess.
...
e2e workspace registers stable Rust 1.78.0, while Aya keeps its pinned nightly.
