...
Rust 1.78.0 toolchain, the upstream minimum for both maintained kernel lines.
`linux.bzl` consumes that selected toolchain and its matching sources; it does
not register a production Rust toolchain itself. Rust-for-Linux actions require
a Linux x86_64 executor even when the kernel target is aarch64. They
intentionally fail for unsupported debug/instrumentation paths or module
