...
func TestSourceIncludeFlagsFailClosed(t *testing.T) {
	...
	for _, flags := range [][]string{
		...
		if _, err := forcedSourceInputs(flags, "drivers/foo.c"); err == nil {
...
func TestForcedSourceInputs(t *testing.T) {
	...
	got, err := forcedSourceInputs(flags, "drivers/foo.c")
	if err != nil {
	...
	got, err = forcedSourceInputs(flags, "arch/x86/entry.S")
	if err != nil {
