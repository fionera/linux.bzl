...
var linuxTargetProfiles = map[string]LinuxTargetProfile{
	...
	"riscv64": {
		Name: "riscv64", Arch: "riscv", Srcarch: "riscv", UTSMachine: "riscv64",
		TargetTriple: "riscv64-linux-gnu", ABISeeds: map[string]string{"CONFIG_64BIT": "y"},
	},
	"ppc64le": {
		Name: "ppc64le", Arch: "powerpc", Srcarch: "powerpc", UTSMachine: "ppc64le",
		TargetTriple: "powerpc64le-linux-gnu",
		ABISeeds: map[string]string{
			"CONFIG_PPC64":             "y",
...
var requiredArchitectureSelectors = map[string][]string{
	...
	"armv7":   {"CONFIG_ARM"},
	"riscv64": {"CONFIG_RISCV"},
	"ppc64le": {"CONFIG_PPC", "CONFIG_PPC64"},
}
...
func (p LinuxTargetProfile) PrepareTargetConfig(raw map[string]string) (map[string]string, error) {
	...
	if p.Name == "ppc64le" && prepared["CONFIG_CPU_BIG_ENDIAN"] == "y" {
		return nil, fmt.Errorf("CONFIG_CPU_BIG_ENDIAN=y contradicts little-endian Linux target profile %q", p.Name)
	}
	return prepared, nil
}
