...
def linux_architectures():
    return [
        struct(
            ...
            compact_vars = {
                "ARCH_CORE": "",
                # Deliberately empty: arch/x86/Makefile's
                # drivers-$(CONFIG_PCI) and drivers-$(CONFIG_PM) lines are
                # already descended into with their config gates, so naming
                # those directories here again would add them a second time,
                # unconditionally.
                "ARCH_DRIVERS": "",
