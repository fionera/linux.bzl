...
def _linux_rust_toolchain_capture_impl(ctx):
    rust_toolchain = ctx.toolchains[_RUST_TOOLCHAIN_TYPE]
    if not hasattr(rust_toolchain, "rustc_srcs"):
        fail(
            (
                "%s selected a rules_rust toolchain without rustc_srcs; " +
                "use a rules_rs toolchain that provides matching Rust sources"
            ) % ctx.label,
        )
    rustc_srcs = rust_toolchain.rustc_srcs
    if not rustc_srcs.to_list():
        fail("%s selected a Rust toolchain without matching rustc sources" % ctx.label)
    return [
        DefaultInfo(files = rust_toolchain.all_files),
        LinuxRustCompilerInfo(
            dylib_ext = rust_toolchain.dylib_ext,
            env = dict(rust_toolchain.env),
            files = rust_toolchain.all_files,
            rustc = rust_toolchain.rustc,
            rustc_srcs = rustc_srcs,
            sysroot = rust_toolchain.sysroot,
            sysroot_anchor = rust_toolchain.sysroot_anchor,
        ),
    ]

linux_rust_toolchain_capture = rule(
    implementation = _linux_rust_toolchain_capture_impl,
    toolchains = [_RUST_TOOLCHAIN_TYPE],
    doc = "Captures one consumer-registered Rust toolchain configuration.",
)

def _linux_rust_toolchain_probe_impl(ctx):
    target = ctx.attr.target_toolchain[LinuxRustCompilerInfo]
    host = ctx.attr.host_toolchain[LinuxRustCompilerInfo]
    out = ctx.actions.declare_file(ctx.label.name + ".json")
    args = ctx.actions.args()
    args.add("-rustc", target.rustc)
    args.add("-host-rustc", host.rustc)
    args.add("-out", out)
    args.add("-minimum", ctx.attr.minimum_version)
    ...
    path_mapped_run(
        ...
        inputs = depset(transitive = [target.files, host.files]),
        outputs = [out],
        arguments = [args],
    ...
    return [
        ...
        LinuxRustToolchainInfo(
            dylib_ext = target.dylib_ext,
            env = dict(target.env),
            files = target.files,
            host = host,
            minimum_version = ctx.attr.minimum_version,
            probe = out,
            rustc = target.rustc,
            rustc_srcs = target.rustc_srcs,
            sysroot = target.sysroot,
            sysroot_anchor = target.sysroot_anchor,
        ),
    ]

linux_rust_toolchain_probe = rule(
    implementation = _linux_rust_toolchain_probe_impl,
    attrs = {
        "host_toolchain": attr.label(
            cfg = "exec",
            mandatory = True,
            providers = [LinuxRustCompilerInfo],
        ),
        "minimum_version": attr.string(mandatory = True),
    ...
    doc = "Validates matching target and host Rust toolchains once per kernel repository.",
)
...
def _run_rustc(
    ...
    runner_args.add(rust_toolchain.probe)
    ...
    path_mapped_run(
        ...
        inputs = depset(
            inputs + [rust_toolchain.probe],
            transitive = [compiler.files] + transitive_tool_inputs,
        ),
        outputs = outputs,
...
def _target_spec(
    ...
    host_toolchain = rust_toolchain.host
...
def _proc_macro(
    ...
    _run_rustc(
        ...
        mapped_directories = {
            host_toolchain.sysroot: directory_anchor(
                host_toolchain.sysroot_anchor,
                host_toolchain.sysroot,
            ),
        },
        transitive_tool_inputs = [host_cc.all_files, host_link.runtime_files],
        version_predicates = version_predicates,
...
def _linux_rust_kernel_sdk_impl(ctx):
    ...
    rust_toolchain = ctx.attr.toolchain_probe[LinuxRustToolchainInfo]
    ...
    crates = {}
    crate_order = []
    rustc_srcs = rust_toolchain.rustc_srcs.to_list()
    ...
    compile_inputs = depset(
        metadata,
        transitive = [rust_toolchain.files],
    )
    ...
    sdk = LinuxRustSdkInfo(
        ...
        rustc_files = rust_toolchain.files,
        rustc_probe = rust_toolchain.probe,
        minimum_rustc_version = rust_toolchain.minimum_version,
